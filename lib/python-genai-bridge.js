...
function parseBridgeOutput(stdout, stderr) {
  ...
  if (!payload.ok) {
    ...
    const statusMatch = String(message).match(/\b(\d{3})\b/);
    if (statusMatch) {
      err.statusCode = Number(statusMatch[1]);
    }
    throw err;
  }
...
function callPythonBridge(request, timeoutMs = Number(process.env.GOOGLE_GENAI_PYTHON_TIMEOUT_MS) || DEFAULT_TIMEOUT_MS) {
...
async function callPythonGenAIEmbed({
  apiKey,
  model,
  contents,
  taskType,
  ...
  const payload = await callPythonBridge({
  ...
  return {
    ...
    pythonBridge: payload.pythonBridge,
  };
}

module.exports = {
  callPythonGenAI,
  callPythonGenAIEmbed,
