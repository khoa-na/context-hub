...
const { PUBLIC_DIR, DOCS_DIR, INTERNAL_DIR, SESSIONS_DIR, SESSION_WEB_DIR, INDEX_PATH, ROOT } = require("../constants");
...
async function ensureStorage() {
  ...
  await fs.mkdir(INTERNAL_DIR, { recursive: true });
  await fs.mkdir(SESSIONS_DIR, { recursive: true });
  await cleanupSessionWebStorage({ recreate: true });
...
async function cleanupSessionWebStorage({ recreate = false } = {}) {
  await fs.rm(SESSION_WEB_DIR, { recursive: true, force: true });
  if (recreate) {
    await fs.mkdir(SESSION_WEB_DIR, { recursive: true });
  }
}
